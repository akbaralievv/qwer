...
import { useGetSubdivOnesQuery, useUpdateSubdivOnesMutation, useRemoveSubdivOnesMutation } from 'src/store/api/subdivOnes';

import Iconify from 'src/components/iconify';

import UpdateSubdivision from '../UpdateSubdivOne';
import CreateSubdivision from '../CreateSubdivOne';
import FacultativeViewModal from '../FacultativeViewModal';

const COLUMN_KEY = 'subdivone_visible_columns';
...
export default function FacultativePage() {
  ...
  const [updateData] = useUpdateSubdivOnesMutation();
  const [removeSubdivision] = useRemoveSubdivOnesMutation();
  const { data, isLoading, refetch } = useGetSubdivOnesQuery(`?sort[0]=${orderBy}:${order}`);
  ...
  const handleDeleteConfirm = async () => {
    if (deleteInfo) {
      try {
        ...
        setSnackbar({ open: true, message: `Факультет "${deleteInfo.title}" удалён`, severity: 'success' });
        refetch();
      } catch {
  ...
  return (
    <Box sx={{ mt: 0, background: '#f6f7f9', minHeight: '100vh', pb: 0 }}>
      ...
      <ModalUi open={deleteDialog} setOpen={setDeleteDialog}>
        <Box p={3}>
          <Typography variant="h6" mb={2}>Удалить факультет?</Typography>
