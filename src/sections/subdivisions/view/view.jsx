...
import { useGetSubdivisionsQuery, useUpdateSubdivisionMutation, useRemoveSubdivisionMutation } from 'src/store/api/subdivsion';

import Iconify from 'src/components/iconify';

import CreateSubdivision from '../CreateSubdivision';
import UpdateSubdivision from '../UpdateSubdivision';

// Модалка для просмотра курса
function SubdivisionViewModal({ open, onClose, data }) {
  const FIELDS = [
    { key: 'id', label: 'ID' },
  ...
  return (
    <ModalUi open={open} setOpen={onClose}>
...
const COLUMN_KEY = 'subdivisions_visible_columns';

const ALL_COLUMNS = [
  { key: 'id', label: 'ID', always: true, width: 80 },
  { key: 'title', label: 'Название', always: true, width: 300 },
  { key: 'createdAt', label: 'Дата создания', always: true, width: 180 },
];
...
export default function SubdivisionsPage() {
  ...
  const [updateData] = useUpdateSubdivisionMutation();
  const [removeSubdivision] = useRemoveSubdivisionMutation();
  const { data, isLoading, refetch } = useGetSubdivisionsQuery(`?sort[0]=${orderBy}:${order}`);

  const allRows = useMemo(() => (data?.data || []).map(row => ({
    ...row.attributes,
    id: row.id,
    createdAt: row.attributes.createdAt?.slice(0, 10) || '',
  })), [data]);
  ...
  const handleDeleteConfirm = async () => {
    if (deleteInfo) {
      try {
        await removeSubdivision(deleteInfo.id).unwrap();
        setSnackbar({ open: true, message: `Курс "${deleteInfo.title}" удалён`, severity: 'success' });
        refetch();
      } catch {
  ...
  return (
    <Box sx={{ mt: 0, background: '#f6f7f9', minHeight: '100vh', pb: 0 }}>
      ...
      <ModalUi open={deleteDialog} setOpen={setDeleteDialog}>
        <Box p={3}>
          <Typography variant="h6" mb={2}>Удалить курс?</Typography>
