...
import { useGetAgregatorsQuery, useRemoveAgregatorMutation } from 'src/store/api/agregatorsApi';
...
const COLUMN_KEY = 'agregators_visible_columns';

const ALL_COLUMNS = [
  { key: 'id', label: 'ID', always: true, width: 80 },
  { key: 'title', label: 'Название', always: true, width: 300 },
  { key: 'inn', label: 'ИНН', always: true, width: 180 },
...
export default function AgregatorsPage() {
  ...
  const [removeAgregator] = useRemoveAgregatorMutation();
  const { data, isLoading, refetch } = useGetAgregatorsQuery(`?sort[0]=${orderBy}:${order}`);
  ...
  const handleDeleteConfirm = async () => {
    if (deleteInfo) {
      try {
        await removeAgregator(deleteInfo.id).unwrap();
        setSnackbar({ open: true, message: `Агрегатор "${deleteInfo.title}" удалён`, severity: 'success' });
        refetch();
      } catch {
  ...
  return (
    <Box sx={{ mt: 0, background: '#f6f7f9', minHeight: '100vh', pb: 0 }}>
      ...
      <ModalUi open={deleteDialog} setOpen={setDeleteDialog}>
        <Box p={3}>
          <Typography variant="h6" mb={2}>Удалить агрегатор?</Typography>
