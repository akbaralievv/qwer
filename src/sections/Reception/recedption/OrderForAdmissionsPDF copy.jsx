...
const PDFFileCopy = ({ formData, positions, row }) => {
  return (
    <Document>
      <Page style={styles.page} size="A4" orientation="portrait">
        ...
        <View style={styles.content}>
          <Text style={[styles.title, { marginTop: 25, marginBottom: 13 }]}>
            БУЙРУК – ПРИКАЗ № {formData?.operations?.data?.at(-1)?.attributes.docNumber || '___'}
          </Text>
          <View style={styles.header}>
            <Text style={styles.miniTitle}>г.Кара-Балта</Text>
            <Text style={styles.miniTitle}>
              {convertToRuFormat(formData?.operations?.data?.at(-1)?.attributes.docDate) || '___'}
            </Text>
          </View>
          ...
          <Text style={[styles.miniTitle, { marginTop: 18 }]}>
            ...
            образования в Кыргызский медико-стоматологический институт
          </Text>
          <Text style={styles.miniTitle}>
            Основание: {formData?.operations?.data?.at(-1)?.attributes?.basedOn || '___'}
          </Text>
          <Text style={[styles.middleTitle, { margin: '10 0 8 0' }]}>ПРИКАЗЫВАЮ:</Text>
          <Text style={styles.miniTitle}>
            Зачислить с{' '}
            {convertToRuFormat(formData?.operations?.data?.at(-1)?.attributes?.periodFrom) || '___'}{' '}
            по{' '}
            {convertToRuFormat(formData?.operations?.data?.at(-1)?.attributes?.periodTo) || '___'} в
            число студентов {row?.division?.data?.attributes.title || '___'} очной формы обучения по
            направлениям:
          </Text>
          <Text style={[styles.miniTitle, { marginTop: 5 }]}>
            1.1 {row?.subdiv_one?.data?.attributes.title || '___'}
          </Text>
          <Text style={[styles.miniTitle, { marginTop: 5 }]}>
            {row.contragent.data.attributes?.name || '___'}
          </Text>
          {/* <Text style={styles.miniTitle}>
            Сумма: {formData?.operations?.data?.at(-1)?.attributes.contract || '___'} сом
          </Text> */}
        </View>
        <View style={styles.content}>
