...
function CreateOrderForAdmissions({
  setOpen,
  setSnackbarOpen,
  setSnackbarMessage,
}) {
  ...
  const isValidForm = () => {
    ...
    if (!contract.trim()) formErrors.contract = 'Незаполнена сумма';
    if (!docDate.trim()) formErrors.docDate = 'Незаполнена дата';
    if (!docNumber.trim()) formErrors.docNumber = 'Незаполнен номер';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        contragent: contragent.idUser,
        division: divisionID,
        subdiv_one: subdiv_oneID,
        service: newUser.service, // <-- id услуги
      };
...
CreateOrderForAdmissions.propTypes = {
  setOpen: PropTypes.func.isRequired,
  setSnackbarOpen: PropTypes.func.isRequired,
  setSnackbarMessage: PropTypes.func.isRequired,
};

export default CreateOrderForAdmissions;
