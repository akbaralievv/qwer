...
export default function OrderForAdmissionsActionsPopover({
  ...
  useEffect(() => {
    console.log('PDF Data loaded:', pdfData?.data?.attributes);
  ...
  return (
    ...
    >
      <MenuItem
        onClick={() => {
          onClose();
          onEdit({ idUser: editData.id, dataUser: editData.attributes });
        }}
      >
      ...
      <MenuItem>
        {pdfData?.data?.attributes ? (
          <PDFDownloadLink
            ...
            fileName={`${pdfData?.data?.attributes?.name || 'Контрагент'}-Приказ о зачислении №${
