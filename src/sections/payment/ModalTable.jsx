...
import { useGetContragentsQuery, useUpdateContragentMutation } from 'src/store/api/contragentAPI';
...
const headTable = [
  ...
  { id: 'resident', label: 'Резидент', mapping: true },
  { id: 'ls', label: 'лицевой счет', mapping: true },
  { id: 'form', label: 'формат обучения', align: 'right', mapping: true },
  { id: 'action', label: 'Действие', align: 'right', mapping: false },
];

export default function ModalTable({ open, setOpen, setContragent, queryParams }) {
  ...
  const { data } = useGetOperationQuery(queryParams);
  ...
  // const { data } = useGetContragentsQuery(
  //   `?sort[0]=id:desc&populate[1]=division&populate[2]=subdiv_one&pagination[page]=${
  //     page + 1
  ...
  const handleDialogOpen = (user) => {
    ...
    setContragent({ dataUser: user.attributes, idUser: user.id });
    setOpen(false);
  };
    ...
    <>
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4">Контрагент</Typography>
